# Development seeds: demo content for local browsing and filter behavior.
#
# Runs via `mix ecto.devsetup`. Requires `seeds.exs` first (roles +
# bootstrap superadmin), then inserts idempotent demo fixtures: regular
# users, categories, series with covers/chapters, view logs, bookmarks,
# notifications, comments, reports, and audit logs. Safe to re-run.
...
# `seeds.exs` evaluates in its own context, so re-read the admin identifier.
...
IO.puts(
  "Devseeded: demo users, 18 series (covers, categories), 288 chapters, view logs, notifications, comments, reports, audit logs"
)
